...
export function EmployeesPage(): JSX.Element {
  ...
  const [draftFilters, setDraftFilters] =
    useState<EmployeeFilterValues>(emptyFilters);
  ...
  function handleSearch(event: FormEvent<HTMLFormElement>): void {
    ...
    setAppliedFilters(buildEmployeeFilters(draftFilters));
  }
  ...
  return (
    <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_340px]">
      ...
      <aside className="app-surface app-shadow h-[80vh] rounded-[28px] border p-5 2xl:sticky 2xl:top-6">
        <form className="flex h-full flex-col gap-3" onSubmit={handleSearch}>
          ...
          <div className="rounded-b-[28px] app-border-soft -mx-5 -mb-5 mt-auto flex gap-3 border-t bg-[var(--color-surface)] p-5">
            <Button
            ...
            <Button
              aria-label={t("employeesPage.filters.addButton")}
              className="h-11 flex-1 rounded-2xl p-0"
              onClick={() => navigate("/employees/new")}
              title={t("employeesPage.filters.addButton")}
              type="button"
              variant="ghost"
            >
