...
- служебные заметки;
- карьеру, стаж, историю назначений и отпуска.
...
После миграций запускается идемпотентный seed. Он создаёт демонстрационное предприятие, отделы, должности, сотрудников, отпуска и начисления через `INSERT OR IGNORE`.

Повторный запуск приложения не создаёт дубликаты. Seed предназначен для разработки и первичного знакомства с системой.
