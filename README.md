...
- ведение вакансий по существующим должностям;
- создание профиля требований из навыков, уровней и весов;
- учёт кандидатов и автоматический расчёт соответствия вакансии;
...
Вакансия обязательно привязывается к существующей должности организационной структуры. Для неё хранятся:

- название и статус;
- формат занятости;
- количество открытых мест;
- описание и текст требований;
- внутренняя заметка;
- набор оцениваемых навыков.

Каждый навык имеет требуемый уровень от `1` до `10` и вес важности от `1` до `5`. Названия навыков внутри одной вакансии не повторяются. Изменение вакансии и её набора навыков выполняется в одной SQLite-транзакции.

### Кандидаты

Кандидат относится к одной вакансии. В карточке кандидата хранятся ФИО, телефон, email, источник, этап подбора и заметка. После выбора вакансии интерфейс показывает её навыки и позволяет оценить кандидата по каждому из них от `0` до `10`.

Соответствие рассчитывается автоматически:

```text
по каждому навыку = min(оценка кандидата / требуемый уровень, 1) × вес
итоговый процент = сумма баллов / сумма весов × 100
```

Отсутствующая оценка считается нулевой. Если у вакансии появляется новый навык, проценты всех её кандидатов пересчитываются при следующем чтении. Удаление навыка удаляет только связанные с ним оценки. SQLite-триггер запрещает записывать кандидату оценку навыка от другой вакансии.
...
| `recruitment:listVacancies` | Поиск и список вакансий                     |
| `recruitment:getVacancy`    | Вакансия вместе с набором навыков            |
| `recruitment:saveVacancy`   | Транзакционное сохранение вакансии и навыков  |
| `recruitment:listCandidates`| Кандидаты с рассчитанным процентом соответствия |
| `recruitment:getCandidate`  | Кандидат, навыки вакансии и оценки            |
| `recruitment:saveCandidate` | Транзакционное сохранение кандидата и оценок  |
...
| `vacancies`                         | Вакансии по должностям                  |
| `vacancy_skills`                    | Навыки, уровни и веса вакансии          |
| `candidates`                        | Кандидаты и этапы подбора               |
| `candidate_skill_scores`            | Оценки кандидатов по навыкам            |
...
007_add_recruitment_module.sql
```
