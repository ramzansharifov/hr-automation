...
import { IconButton, Button, EmptyState, LoadingState } from "../../../shared/ui";
import {
  formatRelatedDate,
...
export function EducationRecordCard({
  ...
  return (
    <RecordShell>
      <RecordCardHeader
        badges={<SoftBadge tone="accent">{getEducationLevelLabel(educationLevel, t)}</SoftBadge>}
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {speciality && (
          <MetaItem icon={<FiAward />} label={t("forms.fields.speciality")} value={speciality} />
        )}
        <MetaItem icon={<FiCalendar />} label={t("forms.fields.startedAt")} value={formatRelatedDate(record.started_at, locale, t)} />
        <MetaItem icon={<FiCalendar />} label={t("forms.fields.endedAt")} value={formatRelatedDate(record.ended_at, locale, t)} />
        <MetaItem icon={<FiFileText />} label={t("forms.fields.documentNumber")} value={valueOrEmpty(getString(record.document_number), t)} />
      </div>
    </RecordShell>
  );
}
  ...
  return (
    <RecordShell>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <MetaItem icon={<FiBriefcase />} label={t("forms.fields.experiencePositionName")} value={valueOrEmpty(getString(record.position_name), t)} />
        <MetaItem icon={<FiCalendar />} label={t("forms.fields.startedAt")} value={formatRelatedDate(record.started_at, locale, t)} />
        <MetaItem
          icon={<FiCalendar />}
      ...
      {responsibilities && (
        <div className="app-surface-muted app-border mt-4 rounded-2xl border p-4">
          <p className="app-muted text-xs font-black uppercase tracking-[0.12em]">{t("forms.fields.responsibilities")}</p>
          <p className="app-text mt-2 whitespace-pre-line text-sm font-medium leading-6">{responsibilities}</p>
        </div>
      )}
    </RecordShell>
  );
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex min-w-0 items-start gap-3.5">
        <span className="app-accent-soft flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl">{icon}</span>
        <div className="min-w-0 pt-0.5">
      ...
      {(onEdit || onDelete) && (
        <div className="flex shrink-0 gap-2 self-end sm:self-auto">
          {onEdit && (
            <IconButton icon={<FiEdit2 />} label={editLabel} onClick={onEdit} size="md" />
          )}
          {onDelete && (
            <IconButton icon={<FiTrash2 />} label={deleteLabel} onClick={onDelete} size="md" tone="danger" />
          )}
        </div>
      )}
    </div>
...
function SoftBadge({ children, tone = "neutral" }: { children: ReactNode; tone?: "accent" | "neutral" }): JSX.Element {
  return (
    <span className={[
...
function MetaItem({ icon, label, value }: { icon: ReactNode; label: string; value: string }): JSX.Element {
  return (
