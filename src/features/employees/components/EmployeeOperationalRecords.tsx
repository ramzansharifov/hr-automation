...
function RecordActionsButtons({ actions }: { actions: RecordActions }): JSX.Element {
  return (
    <div className="flex shrink-0 gap-2">
      <IconButton icon={<FiEdit2 />} label="Редактировать отпуск" onClick={actions.onEdit} size="md" />
      {actions.onDelete && (
        <IconButton icon={<FiTrash2 />} label="Удалить отпуск" onClick={actions.onDelete} size="md" tone="danger" />
      )}
    </div>
  );
}
